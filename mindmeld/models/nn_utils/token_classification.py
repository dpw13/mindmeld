...
class BaseTokenClassification(BaseClassification):
    ...
    def _prepare_labels(self, labels: List[List[int]], max_length: int):
        ...
        def _trim_or_pad(label_sequence):
            if len(label_sequence) > max_length:
                ...
                return label_sequence + [self.params._label_padding_idx] * (
    ...
    def _init_graph(self):
        ...
        try:
            ...
            msg = (
                f"Derived class '{self.name}' must indicate its hidden size for dense layer "
        ...
        self.dense_layer_dropout = nn.Dropout(p=1 - self.params.output_keep_prob)
        if self.params.use_crf_layer:
            ...
            if self.params.num_labels >= 2:
                ...
                self.criterion = nn.CrossEntropyLoss(
                    reduction="mean",
                    ignore_index=self.params._label_padding_idx,
                )
            else:
                msg = (
                    f"Invalid number of labels specified: {self.params.num_labels}. "
        ...
        if targets is not None:
            if self.params.use_crf_layer:
                ...
                mask = torch.as_tensor(
                    targets != self.params._label_padding_idx, dtype=torch.uint8
    ...
    def predict(self, examples):
        ...
        with torch.no_grad():
            for start_idx in range(0, len(examples), self.params.batch_size):
                ...
                batch_predictions = [
                    predictions[: len(_split_lengths)]
                    for predictions, _split_lengths in zip(
                        batch_predictions, batch_data["split_lengths"]
                    )
                ]
    ...
    def predict_proba(self, examples):
        ...
        with torch.no_grad():
            for start_idx in range(0, len(examples), self.params.batch_size):
                ...
                if self.params.use_crf_layer:
                    msg = (
                        f"Prediction probabilities cannot be computed when the param "
...
class LstmForTokenClassification(BaseTokenClassification):
    ...
    def _init_core(self):
        ...
        self.out_dim = (
            self.params.lstm_hidden_dim * 2
            if self.params.lstm_bidirectional
    ...
    def _forward_core(self, batch_data):
        ...
        token_embs = self.lstm_layer(
            token_embs, summed_split_lengths
        )  # [BS, SEQ_LEN, self.out_dim]
...
class CharLstmWithWordLstmForTokenClassification(BaseTokenClassification):
    def _prepare_input_encoder(self, examples, **params):
        if (
            params.get("tokenizer_type")
            and TokenizerType(params.get("tokenizer_type"))
            != TokenizerType.WHITESPACE_AND_CHAR_DUAL_TOKENIZER
        ):
            msg = (
                f"Param 'tokenizer_type' must be "
    ...
    def _init_core(self):
        ...
        self.char_dropout = nn.Dropout(p=1 - self.params.char_lstm_keep_prob)
        self.char_lstm_output_pooling = PoolingLayer(self.params.char_lstm_output_pooling_type)
        char_out_dim = (
            self.params.char_lstm_hidden_dim * 2
            if self.params.char_lstm_bidirectional
    ...
    def _forward_core(self, batch_data):
        ...
        encs = [
            self.char_lstm_output_pooling(enc, seq_len)
        ...
        token_embs = self.lstm_layer(
            char_plus_word_encs, summed_split_lengths
        )  # [BS, SEQ_LEN, self.out_dim]
...
class CharCnnWithWordLstmForTokenClassification(BaseTokenClassification):
    ...
    def _init_core(self):
        ...
        self.char_dropout = nn.Dropout(p=1 - self.params.char_cnn_output_keep_prob)
...
class BertForTokenClassification(BaseTokenClassification):
    def fit(self, examples, labels, **params):
        ...
        if EmbedderType(embedder_type) != EmbedderType.BERT:
            msg = (
                f"{self.name} can only be used with 'embedder_type': "
        ...
        for k, v in safe_values.items():
            ...
            if v != v_inputted:
                msg = (
                    f"{self.name} can be best used with '{k}' equal to '{v}' but found "
    ...
    def _init_core(self):
        self.bert_model = HuggingfaceTransformersContainer(
            self.params.pretrained_model_name_or_path, cache_lookup=False
        ).get_transformer_model()
        if not self.params.update_embeddings:
            for param in self.bert_model.parameters():
                param.requires_grad = False

        self.dropout = nn.Dropout(p=1 - self.params.embedder_output_keep_prob)
        self.out_dim = self.params.emb_dim
    ...
    def _forward_core(self, batch_data):
        ...
        if last_hidden_state is None:
            msg = (
                f"The choice of pretrained bert model "
...
def get_token_classifier_cls(classifier_type: str, embedder_type: str = None):
    try:
        ...
        msg = (
            f"Neural Nets' token classification module expects classifier_type to be amongst"
    ...
    try:
        ...
        msg = (
            f"Neural Nets' token classification module expects embedder_type to be amongst"
            f" {[v.value for v in EmbedderType.__members__.values()]} "
            f" but found '{embedder_type}'."
        )
        raise ValueError(msg) from e

    if embedder_type == EmbedderType.BERT and classifier_type not in [
        TokenClassificationType.EMBEDDER
    ]:
        msg = (
            f"To use the embedder_type '{EmbedderType.BERT.value}', "
