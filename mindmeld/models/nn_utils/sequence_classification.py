...
from .layers import EmbeddingLayer, CnnLayer, LstmLayer, PoolingLayer
...
class EmbedderForSequenceClassification(BaseSequenceClassification):
    ...
    def _init_core(self):
        ...
        self.emb_layer_pooling = PoolingLayer(self.params.embedder_output_pooling_type)
...
class LstmForSequenceClassification(BaseSequenceClassification):
    ...
    def _init_core(self):
        ...
        self.lstm_layer_pooling = PoolingLayer(self.params.lstm_output_pooling_type)
        self.out_dim = (
        ...
        if self.params.embedder_output_pooling_type == "first":
            ...
            if not self.no_pooler_output_exists:
                ...
                if pooler_output is None:
                    ...
                    msg = (
                        f"The transformer model ({self.params.pretrained_model_name_or_path}) "
                    ...
                    msg += (
                        f"Continuing to obtain the first index of the last hidden state due "
            ...
            if self.no_pooler_output_exists:  # not just if-else cond. as its initial value is None
                ...
                if last_hidden_state is None:
                    available_keys = bert_outputs.keys()
                    msg = (
                        f"The choice of pretrained transformer model "
            ...
            encodings = self.emb_layer_pooling(
                last_hidden_state, summed_split_lengths
            )  # [BS, self.out_dim]
...
def get_sequence_classifier_cls(classifier_type: str, embedder_type: str = None):
    try:
        ...
        msg = (
            f"Neural Nets' sequence classification module expects classifier_type to be amongst"
    ...
    try:
        ...
        msg = (
            f"Neural Nets' sequence classification module expects embedder_type to be amongst "
    ...
    if embedder_type == EmbedderType.BERT and classifier_type not in [
        SequenceClassificationType.EMBEDDER
    ]:
        msg = (
