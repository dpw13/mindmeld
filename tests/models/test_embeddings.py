...
def test_embedding_size_is_correct():
    ...
    assert isinstance(token_to_embedding_mapping[b"sandberger"], ndarray)
...
def test_bert_embedder():
    ...
    assert isinstance(encoded_vec, ndarray)
...
def test_custom_embedder():
    class MyCustomEmbedder(Embedder):
        ...
        def encode(self, text_list):
            encoded = [
                [float(self.offset + i)] * self.custom_dim for i, text in enumerate(text_list)
            ]
    ...
    assert os.path.exists(
        os.path.join(
            APP_PATH,
            ".generated/indexes",
            "my_another_custom_embedder_0_cache.pkl",
        )
    )
