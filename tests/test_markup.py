...
@pytest.mark.parametrize(
    ...
    [
        (
            "let's meet {after 3:00|sys_interval}",
            [
                ("2020-06-12T15:00:00.000-07:00", None),
                ("2020-06-13T03:00:00.000-07:00", None),
                ("2020-06-13T15:00:00.000-07:00", None),
            ],
        ),
        (
            "let's meet {until 3|sys_interval}",
            [
                (None, "2020-06-12T15:00:00.000-07:00"),
                (None, "2020-06-13T03:00:00.000-07:00"),
                (None, "2020-06-13T15:00:00.000-07:00"),
            ],
        ),
        (
            ...
            [
                (
                    "2020-06-12T11:00:39.000-07:00",
                    "2020-06-12T15:00:01.000-07:00",
                )
            ],
        ),
    ],
...
def test_entity_values(query_factory, query, expected_values):
    ...
    processed_query = markup.load_query(
        query, query_factory, query_options={"timestamp": 1591984839}
    )
    entity = processed_query.entities[0]
...
def test_load_system_role(query_factory):
    ...
    text = "What stores are open between {3|sys_time|open_hours} and " "{5|sys_time|close_hours}"
...
def test_dump_role(query_factory):
    ...
    markup_text = (
        "What stores are open between {3|sys_time|open_hours} and " "{5|sys_time|close_hours}"
    )
...
def test_dump_multi_nested(query_factory):
    ...
    markup_text = (
        "show me houses {between {600,000|sys_number} and " "{1,000,000|sys_number} dollars|price}"
    )
...
def test_dump_group_with_role(query_factory):
    ...
    entity_text = (
        "a {large|size} {latte|dish-type|beverage} with " "{nonfat milk|option|beverage} please"
    )
...
def test_dump_group_nested(query_factory):
    ...
    group_text = (
        "Order [one large Tesora with [medium " "cream|option] and [medium sugar|option]|product]"
    )
...
def test_dump_group_nested_2(query_factory):
    ...
    role_text = "Can I get {one|quantity} curry sauce " "with my rice ball with house salad"
    group_text = "Can I get [[one curry sauce|option] " "with my rice ball with house salad|dish]"
...
def test_load_dump_2(query_factory):
    """Tests that load_query and dump_query are reversible"""
    text = (
        "i'm extra hungry get me a {chicken leg|dish}, [{1|quantity} "
