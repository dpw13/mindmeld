...
# English Tests
class TestAutoAnnotatorEnglish:
    @pytest.fixture(scope="class")
    def en_mla(self, kwik_e_mart_app_path: str) -> MultiLingualAnnotator:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # SpaCey classifies "the 1st" as a date rather than an ordinal
            # so we don't get a chance to parse it as such.
            # TODO: SpaCey training: "the 1st born" should be ordinal, not date
            ("Who is the first born?", "first", 1),
            ("5th", "5th", 5),
        ],
    )
    def test_en_ordinal_parse(self, en_mla, query: str, body: str, value: str, caplog):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # SpaCey is a little over-eager on large numbers as dates. With
            # text like "nine thousand and eight" SpaCey appears to always
            # interpret that as a year rather than an actual number.
            # TODO: SpaCey training: "nine thousand and eight stories" should be cardinal,
            #   not date
            ("one hundred and eight stories", "one hundred and eight", 108),
        ],
    )
    def test_en_cardinal_parse(self, en_mla, query, body, value, caplog):
        caplog.set_level(logging.DEBUG)
    ...
    def test_en_percent_parse(self, en_mla, query, value):
    ...
    def test_en_distance_parse(self, en_mla, query, unit, value):
    ...
    def test_en_weight_parse(self, en_mla, query, unit, value):
    ...
    def test_en_person_parse(self, en_mla, query, value):
    ...
    def test_en_money_parse(self, en_mla, query, unit, value, caplog):
        caplog.set_level(logging.DEBUG)
    ...
    def test_en_duration_parse(self, en_mla, query, unit, value, caplog):
        caplog.set_level(logging.DEBUG)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # This may or may not be a SpaCy issue. It's not clear how this originally worked.
            # TODO: SpaCy training: more flexible relative date detection
            # ("first thursday of 2015", "day", "2015-01-01T00:00:00.000-08:00"),
        ],
    )
    def test_en_time_parse(self, en_mla, query, grain, value, caplog):
        caplog.set_level(logging.DEBUG)
        _check_match(en_mla, query, "sys_time", None, value, None, grain)

    def test_en_rel_time_parse(self, en_mla, caplog):
        caplog.set_level(logging.DEBUG)
        query = "next wednesday"
        grain = "day"
        dt = datetime.datetime.now(tz=pytz.FixedOffset(-7 * 60)) + relativedelta.relativedelta(
            days=+6, weekday=relativedelta.WE, hour=0, minute=0, second=0, microsecond=0
        )
...
# Spanish Tests
class TestAutoAnnotatorSpanish:
    @pytest.fixture(scope="class")
    def es_mla(self, kwik_e_mart_app_path: str) -> MultiLingualAnnotator:
        return MultiLingualAnnotator(
            app_path=kwik_e_mart_app_path,
            language="es",
            locale=None,
    ...
    def test_es_money_parse(self, es_mla, query, unit, value):
    ...
    def test_es_duration_parse(self, es_mla, query, unit, value):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Duckling seems to have issues with larger Spanish numbers. Even with
            # "novecientos ocho" it recognizes "900" and "8" and won't combine them.
            # Proper behavior may require translation first.
            # TODO: duckling fails with large spanish cardinals
            # ("novecientos ocho mujeres", "novecientos ocho", 908),
            # ("nueve mil ocho mujeres", "nueve mil ocho", 9008),
        ],
    )
    def test_es_cardinal_parse(self, es_mla, query, body, value):
    ...
    def test_es_ordinal_parse(self, es_mla, query, body, value):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # SpaCy is producing a MISC token instead of a person
            # TODO: SpaCy doesn't recognize Sarah as a person's name en espanol
            ("Llama a Maria ahora", "Maria"),
            ("¿Quien es Robert?", "Robert"),
            ("¿Samantha está enferma hoy?", "Samantha"),
            ("Richard, necesito tu ayuda", "Richard"),
        ],
    )
    def test_es_person_parse(self, es_mla, query, value):
    ...
    def test_es_time_parse(self, es_mla, query, grain, value):
        _check_match(es_mla, query, "sys_time", None, value, None, grain)


# French Tests
class TestAutoAnnotatorFrench:
    @pytest.fixture(scope="module")
    def fr_mla(self, kwik_e_mart_app_path: str) -> MultiLingualAnnotator:
        return MultiLingualAnnotator(
            app_path=kwik_e_mart_app_path,
            language="fr",
            locale=None,
    ...
    def test_fr_money_parse(self, fr_mla, query, unit, value):
    ...
    def test_fr_duration_parse(self, fr_mla, query, unit, value):
    ...
    def test_fr_cardinal_parse(self, fr_mla, query, body, value):
    ...
    def test_fr_ordinal_parse(self, fr_mla, query, body, value):
        _check_match(fr_mla, query, "sys_ordinal", body, value, None, None)

    @pytest.mark.parametrize(
        "query, value",
        [
            ("Laissons Jason rejoindre la réunion", "Jason"),
            ("Où est Robert?", "Robert"),
            # Same problem as "Sarah" above
            # TODO: SpaCY doesn't recognize the name Samantha en Francais
            ("Camille est-elle malade aujourd'hui?", "Camille"),
            ("Richard, j'ai besoin de ton aide", "Richard"),
        ],
    )
    def test_fr_person_parse(self, fr_mla, query, value):
    ...
    def test_fr_time_parse(self, fr_mla, query, grain, value):
